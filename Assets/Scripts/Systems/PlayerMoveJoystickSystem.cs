...
namespace Labirintus.ECS
{
    ...
    /// </summary>
    internal sealed class PlayerMoveJoystickSystem : IEcsRunSystem
    {
        ...
        public void Run()
        {
            ...
            if (_filterMoving.IsEmpty())
            {
                ...
                foreach (var i in _filter)
                {
                    ...
                    int directionAnim = 3; //по умолчанию вниз
                    //Х преобладает
                    if (Mathf.Abs(directionMoving.x) > Mathf.Abs(directionMoving.y))
                    {
                        if (directionMoving.x > 0) directionAnim = 2; //вправо
                        if (directionMoving.x < 0) directionAnim = 4;//влево
                    }
                    else if (Mathf.Abs(directionMoving.y) > Mathf.Abs(directionMoving.x))
                    {
                        if (directionMoving.y > 0) directionAnim = 1; //вверх
                        if (directionMoving.y < 0) directionAnim = 3; //вниз
                    }
                    ...
                    playerRef.animator.SetInteger("Idle", directionAnim);
            ...
            else //если уже двигается
            {
                foreach (var i in _filterPlayer)
                {
                    ...
                    transform.position += directionMoving.normalized * _config.PlayerMoveSpeed * Time.deltaTime;
                    ...
                    //        _world.NewEntity().Get<NextMazeButtonViewComponent>().isView = true; //показываем кнопку выхода
                    //    }
                    if(directionMoving.magnitude <= 0.1f)
